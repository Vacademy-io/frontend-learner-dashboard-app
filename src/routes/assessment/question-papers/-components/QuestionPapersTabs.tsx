...
export const QuestionPapersTabs = () => {
  ...
  const [selectedQuestionPaperFilters, setSelectedQuestionPaperFilters] =
    useState<Record<string, FilterOption[]>>({});
  const [searchText, setSearchText] = useState("");
  const [pageNo, setPageNo] = useState(0);
  const [questionPaperList, setQuestionPaperList] = useState(null);
  const [questionPaperFavouriteList, setQuestionPaperFavouriteList] =
    useState(null);
  const [isLoading, setIsLoading] = useState(false);
  const setHandleRefetchData = useRefetchStore(
    (state) => state.setHandleRefetchData
  );

  const { YearClassFilterData, SubjectFilterData } =
    useFilterDataForAssesment(instituteDetails);
      ...
      <div className="flex flex-wrap items-center justify-between gap-8">
        <div className="flex flex-wrap gap-8">
          {questionPaperList !== null &&
            questionPaperFavouriteList !== null && (
              <TabListComponent
          ...
          <QuestionPapersFilter
            ...
            onSelectionChange={(items) =>
              handleFilterChange("level_ids", items)
            }
          />
          <QuestionPapersFilter
            label="Subject"
