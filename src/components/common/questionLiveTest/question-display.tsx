...
import { Checkbox } from "@/components/ui/checkbox";
import { useState, useEffect } from "react";
import { getAssessmentData, saveAssessmentData } from "@/routes/assessment/examination/-utils.ts/capacitor";
...
export function QuestionDisplay() {
  const {
    ...
    updateQuestionTimer,
    moveToNextQuestion,
    initializeStoreFromSavedData,
  } = useAssessmentStore();

  const isTimeUp = sectionTimers[currentSection]?.timeLeft === 0;

  useEffect(() => {
    // Initialize store from saved data on component mount
    (async () => {
      const savedData = await getAssessmentData();
      if (savedData) {
        initializeStoreFromSavedData(savedData);
      }
    })();
  }, [initializeStoreFromSavedData]);

  useEffect(() => {
    // Save assessment data whenever relevant state changes
    (async () => {
      const stateToSave = {
        answers,
        sectionTimers,
      ...
      await saveAssessmentData(stateToSave);
    })();
  }, [answers, sectionTimers, questionTimers, questionStates]);
    ...
    <div className="space-y-6 mx-auto">
      <div className="flex flex-col items-start justify-between">
        <div className="w-full sm:w-3/4">
          ...
          <p className="text-lg text-gray-800">{currentQuestion.questionName}</p>
        </div>
        <div className="flex gap-2 mt-4 md:mt-4 sm:mt-0 w-full sm:w-auto justify-between">
          <Button
            variant="outline"
