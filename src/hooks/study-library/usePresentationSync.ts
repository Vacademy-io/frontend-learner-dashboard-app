...
export const usePresentationSync = () => {
    ...
    const syncPresentationTrackingData = async () => {
        console.log(
            "🚀 [usePresentationSync] Starting presentation sync process"
        );
        try {
            ...
            if (!value) {
                console.log(
                    "📭 [usePresentationSync] No tracking data found in storage"
                );
                return;
            }

            console.log(
                "📊 [usePresentationSync] Found tracking data, processing..."
            );
            ...
            for (const activity of activities) {
                ...
                console.log(
                    `🔄 [usePresentationSync] Processing unsynced activity: ${activity.activity_id}`
                );
                ...
                const apiPayload: TrackingDataType = {
                    ...
                    videos: null,
                    documents:
                        activity.view_sessions.length > 0
                            ? activity.view_sessions.map((session, index) => ({
                                  id: session.id,
                                  start_time_in_millis:
                                      session.start_time_in_millis,
                                  end_time_in_millis:
                                      session.end_time_in_millis,
                            ...
                            : [
                                  {
                                      id: activity.activity_id,
                                      start_time_in_millis:
                                          activity.start_time_in_millis,
                                      end_time_in_millis:
                                          activity.end_time_in_millis,
                ...
                try {
                    ...
                    if (
                        ...
                        activity.view_sessions.length >= 1
                    ) {
                        console.log(
                            `📡 [usePresentationSync] Making API call for activity: ${activity.activity_id}`
                        );
                    ...
                    console.error(
                        "❌ [usePresentationSync] API call failed:",
                        error
                    );
                    updatedActivities.push(activity);
                }
