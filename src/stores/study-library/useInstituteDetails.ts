...
export const useInstituteDetailsStore = create<InstituteDetailsStore>(
    (set, get) => ({
        ...
        getPackageWiseLevels: (params?: { sessionId?: string }) => {
            ...
            const filteredBatches =
                instituteDetails.batches_for_sessions.filter((batch) => {
                    if (params?.sessionId) {
                        return batch.session.id === params.sessionId;
                    }
                    return true;
                ...
                if (!packageGroups[packageId]) {
                    packageGroups[packageId] = {
                        package_dto: {
                            ...
                            thumbnail_file_id:
                                batch.package_dto.thumbnail_id || "",
                        },
                        level: [],
                    };
                }
        ...
        getSessionNameById: (sessionId: string) => {
            ...
            const session = instituteDetails.sessions.find(
